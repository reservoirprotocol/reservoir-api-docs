...
Reservoir's bids table contains an extensive number of records. Sequentially paginating through each request would entail a prolonged duration. To mitigate this, the [Bids](/reference/getordersbidsv5) API introduces two supplementary query parameters: **startTimestamp**, **endTimestamp**.
...
We suggest setting a polling interval of one request per minute. This strategy allows sufficient time for processing around 1000 bids, minimizing the need to filter out duplicates. Based on your unique use case, you may adjust this interval to be shorter or longer as needed.
